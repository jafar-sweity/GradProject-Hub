...
if (!apiKey || !secret) {
  throw new Error(
    "API_GETSTREAM_PUBLISHABLE_KEY and API_GETSTREAM_SECRET_KEY must be defined"
  );
}
...
router.post("/sendScheduleMeetingInfo", async (req, res) => {
  ...
  <div style="padding: 20px;">
    ...
    <p style="font-size: 16px; line-height: 1.6;">
      You have a meeting scheduled by <strong style="color: #1E90FF;">${meetingCreator}</strong> on <strong style="color: #1E90FF;">${new Date(
    meetingDate
    ...
    for (const email of emails) {
      ...
      if (user?.notificationToken) {
        await sendPushNotification(
          ...
          "Meeting Scheduled",
          `You have a meeting scheduled by ${meetingCreator} on ${new Date(
            meetingDate
          ).toLocaleString()}.`
        );
      }
