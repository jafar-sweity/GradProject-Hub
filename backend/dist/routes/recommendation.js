...
const professors = [
    {
        name: "Manar Qamhieh",
        position: "Director of the Department of Electrical and Computer Engineering",
    ...
    {
        name: "Anas Toma",
        position: "Assistant Professor, Coordinator of the Master's program in Artificial Intelligence",
...
function createResearchSummary(researchText) {
    if (!researchText)
        return ""; // Handle empty text
...
function precomputeTfIdf(professors) {
...
function recommendProfessors(query, professors) {
    ...
    const filteredScores = similarityScores.filter((item) => item.score >= threshold);
